...
def get_font_ingame(size):  
    return pygame.font.Font("../../assets/font.ttf", size)

def get_title_font_ingame(size):  
    return pygame.font.Font("../../assets/Jersey20-Regular.ttf", size)
...
class Example(Base):
    def __init__(self, screen_size):
        ...
        self.collision_zoom_duration = 5000 
    ...
    def initialize(self):
        ...
        self.wave_material.add_uniform("float", "tileU", 6.0)  
        self.wave_material.add_uniform("float", "tileV", 6.0)  
        ...
        self.player_material = TextureMaterial(texture=Texture(
            file_name="../../images/gradiente1.jpg"))  
        ...
        self.distance_update_interval = 2000  
    ...
    def handle_collision_zoom(self, obstacle):
        ...
        new_camera_pos = [
            ...
            (player_pos[1] + obstacle_pos[1]) / 2 + 3,  
            player_pos[2] - 15  
        ]
    ...
    def adjust_speed_and_obstacle_frequency(self):
        if self.distance % 5 == 0:
            ...
            self.obstacle_chance = max(5, self.obstacle_chance - 2)  
    ...
    def handle_input(self, keys=None):  
        ...
        if keys[K_RIGHT]:  
            if not self.lane_switching and not self.sliding and not self.jumping:
                ...
                self.lane_switching = True
                self.switch_timer = pygame.time.get_ticks()
        if keys[K_UP] and not self.jumping and not self.sliding:  
            self.jumping = True
            self.jump_start_y = self.player.get_position()[1]
            self.jump_time = 0
        if keys[K_DOWN] and not self.sliding and not self.jumping:  
            self.sliding = True
            self.slide_time = 0
    ...
    def move_floor(self):
        ...
        translation_vector = [0, -self.base_speed, 0]  
    ...
    def show_pause_menu(self):
        ...
        while self.is_game_paused:
            ...
            PAUSE_TEXT = get_title_font_ingame(300).render("PAUSED", True, "White")
            ...
            RESTART_BUTTON = Button(image=pygame.image.load("../../assets/Play Rect.png"), pos=(SCREEN_SIZE[0] // 2, SCREEN_SIZE[1] // 2 + 150), 
                            text_input="RESTART",font=get_font_ingame(75), base_color="#d7fcd4", hovering_color="Blue")
            QUIT_BUTTON = Button(
                image=pygame.image.load("../../assets/Quit Rect.png"),
                pos=(SCREEN_SIZE[0] // 2, SCREEN_SIZE[1] // 2 + 300),
                text_input="QUIT",
                font=get_font_ingame(75),
                base_color="#d7fcd4",
                hovering_color="Blue"

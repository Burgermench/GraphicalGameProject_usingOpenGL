...
class Example(Base):
    def initialize(self):
        ...
        lane_count = 3
        lane_spacing = 50
        ground_geometry = RectangleGeometry(width=lane_width * lane_count + lane_spacing * (lane_count - 1), height=100)
    ...
    def apply_gravity(self):
        ...
        if self.jumping:
            ...
            self.jump_time += 1
            jump_progress = min(self.jump_time / self.jump_duration, 1)  # Adjust jump speed based on duration
    ...
    def add_obstacle(self):
        ...
        obstacle_lane = choice([-1.5, 0, 1.5])  # Randomly select a lane for the obstacle
        obstacle_x = obstacle_lane * 2  # Adjust obstacle position based on the lane
        obstacle = Mesh(obstacle_geometry, obstacle_material)
        obstacle.set_position([obstacle_x, 0, -20])  # Position far away in the z-axis
    ...
    def slide(self):
        current_pos = self.kite.get_position()
        new_y = max(0, current_pos[1] - self.gravity)  # Ensure kite doesn't go beneath the ground
        self.kite.set_position([current_pos[0], new_y, current_pos[2]])
    
    def move_obstacles(self):
        new_obstacles = []
        for obstacle in self.obstacles:
            obstacle.translate(0, 0, 0.2)  # Move obstacle towards the player using Object3D method
            if obstacle.global_position[2] < 30:  # Keep obstacles within a certain range
    ...
    def run(self):
        ...
        # Inside the main loop
            keys = pygame.key.get_pressed()
            self.handle_input(keys)  # Pass the 'keys' obtained from pygame.key.get_pressed() to handle_input
